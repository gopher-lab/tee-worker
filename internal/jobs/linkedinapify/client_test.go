...
var _ = Describe("LinkedInApifyClient", func() {
	...
	Describe("SearchProfiles", func() {
		...
		It("should correctly unmarshal valid profile items", func() {
			profileJSON, _ := json.Marshal(map[string]any{
				...
				"topSkills":               "Go, Python, JavaScript",
			...
			dataset := &client.DatasetResponse{
				Data: client.ApifyDatasetData{
					Items: []json.RawMessage{profileJSON},
				},
			}
			mockClient.RunActorAndGetResponseFunc = func(actorID apify.ActorId, input any, cursor client.Cursor, limit uint) (*client.DatasetResponse, client.Cursor, error) {
				return dataset, "next", nil
			...
			results, _, cursor, err := linkedinClient.SearchProfiles("test-worker", &args, client.EmptyCursor)
			Expect(err).NotTo(HaveOccurred())
			Expect(cursor).To(Equal(client.Cursor("next")))
			Expect(results).To(HaveLen(1))
			Expect(results[0].ID).To(Equal("test-id"))
			Expect(results[0].FirstName).To(Equal("John"))
			Expect(results[0].LastName).To(Equal("Doe"))
			Expect(results[0].Headline).To(Equal("Software Engineer"))
		})
